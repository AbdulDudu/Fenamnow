import { Bucket, CameraFaceType, MediaFileType } from "@fenamnow/types/files";
...
export const upload = async ({
  ...
  if (file) {
    path =
      ...
      `${fileType}s/${
        fileType == "file"
          ? (file as DocumentPicker.DocumentPickerAsset).name.split(".").shift()
          : file.uri.split("/").pop()?.split(".").shift() || Date.now()
      }.${ext}`;
