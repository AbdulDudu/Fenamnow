...
export default function AgentVerificationPage() {
  ...
  const { data: agentApplicationData, error } = useQuery({
    queryKey: [session],
    queryFn: async () => {
      return await supabase.from("agent_applications").select("*").eq("user_id", session.user.id).single();
    },
    enabled: !!session,
  });
                    ...
                    onEvent: function (message) {
                      switch (message) {
                        case MESSAGES.FINISHED:
                          isMobile
                            ? window.ReactNativeWebView.postMessage("Your documents were submitted successfully")
                            : toast("Your documents were submitted successfully", {
                                duration: 3000,
        ...
        {agentApplicationData?.data && (
          <div className="flex w-[400px] flex-col items-center space-y-6">
            <p className="text-3xl font-semibold">
              We're verifying your identity. We'll reach out to you if we need any extra information.
            </p>
            <Button asChild>
